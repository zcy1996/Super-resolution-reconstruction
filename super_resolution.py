...
class SuperResolution:
	...
	def build_new_layer(self):
		self.x = tf.placeholder(tf.float32, shape=[None, None, None, self.channels], name="X1")
		self.y = tf.placeholder(tf.float32, shape=[None, None, None, self.channels], name="Y1")

		# H-1 conv
		with tf.variable_scope("W-3_conv"):
			self.Wm1_conv = util.weight([self.cnn_size, self.cnn_size, self.channels, self.feature_num],
										stddev=self.weight_dev, name="conv_W1", initializer=self.initializer)
			self.Bm1_conv = util.bias([self.feature_num], name="conv_B1")
			Hm1_conv = util.conv2d_with_bias(self.x, self.Wm1_conv, self.cnn_stride, self.Bm1_conv, add_relu=True,
											 name="H1")
		...
		if self.summary:
			...
			tf.summary.image("W-3/" + self.model_name, Wm1_transposed, max_outputs=self.log_weight_image_num)
			util.add_summaries("B-3", self.model_name, self.Bm1_conv, mean=True, max=True, min=True)
			util.add_summaries("W-3", self.model_name, self.Wm1_conv, mean=True, max=True, min=True)

			util.add_summaries("B2", self.model_name, self.B0_conv, mean=True, max=True, min=True)
			util.add_summaries("W2", self.model_name, self.W0_conv, mean=True, max=True, min=True)
	...
	def print_status(self, mse):
		...
		if self.step == 0:
			...
			# str_temp = "temp/"+str(self.epochs_completed)
			# f = open(str_temp,"w")
			# f.closed()
